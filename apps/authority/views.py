...
from apps.vocabulary.models import AuthorityCategory, AuthorityProfile
...
def retrieve_authority_list(a_id=None):
    """
    ...
    if a_id:
        try:
            category = AuthorityCategory.objects.get(id=a_id)
        except AuthorityCategory.MultipleObjectsReturned:
            category = AuthorityCategory.objects.filter(id=a_id)[0]
        except AuthorityCategory.DoesNotExist:
            return None
        category = [category]
...
def get_authority_list(request, a_id=None, **kwargs):
    """
    ...
    if a_id: # `id` is a node id here.
        try: # And it can only be int!
            a_id = int(a_id)
        except:
            raise Http404

    result = retrieve_authority_list(a_id)
