...
from apps.userprofile.forms import UserProfileForm, UserpicForm

from sezam.settings import MEDIA_ROOT, THUMBNAIL_SIZE, PAGINATE_BY


def user_profile(request, p_id=None, **kwargs):
    """
    ...
    user = get_object_or_404(User, pk=int(p_id))
    if profile:
        try:
    ...
    return render_to_response(template, { 'usr': user,
        'user_profile': u_profile, 'user_message': user_message,
...
def user_profile_update(request, p_id=None, **kwargs):
    """
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('save_changes', None):
            ...
            if form.is_valid():
                try:
                    u_profile = form.save()
                    return redirect(reverse('user_profile', args=(str(p_id),)))
    ...
    return render_to_response(template, {'user_message': user_message,
        'user_profile': u_profile, 'form': form,
        'page_title': _(u"Update profile ") + u_profile.user.get_full_name()},
        context_instance=RequestContext(request))


def user_set_userpic(request, p_id=None, **kwargs):
    """
    Update userpic.
    """
    template = kwargs.get('template', 'user.html')
