...
def hide_all_emails_filter(value):	
    ...
    def hide_email_callback1(matchobj):
        """
        Alternative emails hide.
        """
        return '@'.join(['*' * len(a) for a in matchobj.group(0).split('@')])

    pattern= '<a href="mailto:([\.\w-]+@[\w-]+\.[\w-]+)">([^<]+)</a>'
    value= re.sub(pattern, mailto_hide_callback, value)

    pattern= r"([\.\w-]+@[\w-]+[\.\w-]+)"
    value= re.sub(pattern, hide_email_callback1, value)
