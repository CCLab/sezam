...
from django.db.models import ForeignKey, CharField, BooleanField, ManyToManyField, \
    DateTimeField, OneToOneField, Manager
...
class PIARequestManager(Manager):
    """
    Active PIARequest manager
    """

    def get_query_set(self):
        return super(PIARequestManager, self).get_query_set().filter(active=True)
...
class PIARequest(GenericEvent):
    ...
    objects = Manager()
    active_objects = PIARequestManager()
