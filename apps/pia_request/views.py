...
def save_request_draft(request, d_id=None, **kwargs):
    """
    ...
    if form.is_valid():
        ...
        if result['draft']:
            ...
            if d_id: # Report only if it's draft save, not the first preview.
                user_message = update_user_message(user_message,
...
def get_request_draft(request, d_id, **kwargs):
    """
    Return request draft data for display.
    """
    try:
        draft = PIARequestDraft.objects.get(pk=int(d_id))
    except:
        raise Http404
    ...
    return {'template': template, 'form': form,
            'request_id': d_id, 'user_message': user_message}
...
@login_required
def confirm_overdue(request, r_id):
    """
    confirm overdue
    """

    try:
        pia_request = PIARequest.objects.get(id=r_id)
        pia_request.overdue_confirm_send = True
        pia_request.save()
    except PIARequest.DoesNotExist:
        return Http404

@login_required
def confirm_long_overdue(request, r_id):
    """
    confirmg long overdue
    """

    try:
        pia_request = PIARequest.objects.get(id=r_id)
        pia_request.long_overdue_confirm_send = True
        pia_request.save()
    except PIARequest.DoesNotExist:
        return Http404
